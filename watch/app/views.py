    ...
    if 'shop' in req.session:
        if req.method == 'POST':
            try:
                ...
                dis = req.POST.get('descrip')
                price = req.POST.get('price')
                off_price = req.POST.get('off_price')
                stock = req.POST.get('stock')
                cate = req.POST.get('category')
                file = req.FILES.get('img')

                # Check if all required fields are provided
                if not all([pid, name, dis, price, off_price, stock, cate, file]):
                    return HttpResponse("All fields are required.", status=400)

                # Validate and fetch category
                try:
                    cate_id = int(cate)  # Ensure cate is a valid number
                    cat = Category.objects.get(pk=cate_id)
                except ValueError:
...
def update_stock(request, product_id):
    if request.method == "POST":
        try:
            ...
            # Ensure stock does not increase accidentally
            if new_stock < product.stock:
                product.stock = new_stock
                product.save()
                return JsonResponse({"success": True, "new_stock": new_stock})
            else:
                return JsonResponse({"success": False, "error": "Invalid stock update"})
